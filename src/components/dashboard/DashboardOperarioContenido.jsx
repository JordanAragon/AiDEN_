    ...
    <article className="space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Kpi
          ...
          detail="Revisa las zonas afectadas"
          tone="amber"
        />
      </section>
              ...
              >
                <span className="mt-0.5 shrink-0">
                  {t.estado === "Completada" ? (
                    ...
                    <span
                      className={`block h-[18px] w-[18px] rounded-full border-2 ${t.prioridad === "Alta" ? "border-red-400" : "border-slate-300"}`}
                    />
                  )}
                </span>
                <span className="min-w-0 flex-1">
                  <span
                    className={`block text-sm font-medium ${t.estado === "Completada" ? "text-slate-400 line-through" : "text-slate-700"}`}
                  >
                    {t.titulo}
                  </span>
        ...
        <article className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
          ...
          <p className="mt-2 text-sm leading-6 text-amber-900/80">
            {misIncidencias.length
              ? `Hay ${misIncidencias.length} incidencia${misIncidencias.length !== 1 ? "s" : ""} abierta${misIncidencias.length !== 1 ? "s" : ""} en tus lotes.`
