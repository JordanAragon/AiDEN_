...
export default function DashboardSupervisorContenido() {
  ...
  const pendientes = data.tareas.filter((t) => t.estado !== "Completada");
  const atrasadas = pendientes.filter((t) => t.fecha && t.fecha < new Date().toISOString().slice(0, 10));
  const alertas = [
    ...data.calidad
      .filter((r) => (r.estado ?? r.estadoManual) !== "Cerrada" && r.prioridad === "Alta")
      .map((r) => ({ text: `Calidad: ${r.codigo || r.id} · ${r.descripcion}`, ruta: "/calidad" })),
    ...data.inventario
      .filter((r) => Number(r.stock) <= Number(r.minimo))
      .map((r) => ({ text: `Inventario: ${r.nombre} bajo mínimo`, ruta: "/inventario" })),
    ...data.ambiental
      .filter((r) => Number(r.temperatura) < Number(cfg.tempMin) || Number(r.temperatura) > Number(cfg.tempMax) || Number(r.humedad) < Number(cfg.humMin) || Number(r.humedad) > Number(cfg.humMax))
      .map((r) => ({ text: `Ambiental: ${r.zona} fuera de rango`, ruta: "/ambiental" })),
  ];
  const carga = [...new Set(data.tareas.map((t) => t.responsable).filter(Boolean))].map((nombre) => ({
    nombre,
    total: data.tareas.filter((t) => t.responsable === nombre && t.estado !== "Completada").length,
  }));
  ...
  return (
    <article className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <section>
          <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-emerald-700">AiDEN / coordinación</p>
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-950">Centro de supervisión</h1>
          <p className="mt-1 max-w-2xl text-sm text-slate-500">Coordina la ejecución diaria: asigna trabajo, detecta bloqueos, valida incidencias y mantiene la operación en movimiento.</p>
        </section>
        <button type="button" onClick={() => navigate("/personal")} className="inline-flex items-center gap-2 rounded-xl bg-emerald-700 px-4 py-2.5 text-sm font-semibold text-white hover:bg-emerald-800">
          <ClipboardList size={15} /> Asignar trabajo
        </button>
      </header>
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Kpi icon={Sprout} label="Lotes activos" value={data.lotes.filter((l) => l.estado !== "Inactivo").length} detail={`${data.lotes.filter((l) => l.etapa === "Cosecha").length} en cosecha`} />
        <Kpi icon={ClipboardList} label="Trabajo pendiente" value={pendientes.length} detail={atrasadas.length ? `${atrasadas.length} atrasadas` : "Sin tareas vencidas"} tone={atrasadas.length ? "red" : "blue"} />
        <Kpi icon={AlertTriangle} label="Alertas" value={alertas.length} detail={`Operación, ambiente y calidad · ${cfg.tempMin}–${cfg.tempMax} °C`} tone={alertas.length ? "amber" : "green"} />
        <Kpi icon={Package} label="Insumos bajo mínimo" value={data.inventario.filter((r) => Number(r.stock) <= Number(r.minimo)).length} detail="Coordina reposición" tone="amber" />
      </section>
      <section className="grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <section className="mt-5 space-y-4">
            {carga.map((persona) => <section key={persona.nombre}><section className="mb-1 flex justify-between text-xs"><span className="font-medium text-slate-700">{persona.nombre}</span><span className="font-semibold text-slate-500">{persona.total} pendientes</span></section><section className="h-2 rounded-full bg-slate-100"><span className="block h-full rounded-full bg-emerald-600" style={{ width: `${Math.min(100, persona.total * 14)}%` }} /></section></section>)}
            {!carga.length && <p className="text-sm text-slate-400">Aún no hay tareas asignadas.</p>}
          </section>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <section className="mt-4 space-y-2">
            {alertas.slice(0, 5).map((a) => <button type="button" key={a.text} onClick={() => navigate(a.ruta)} className="w-full rounded-xl border border-slate-100 bg-slate-50 p-3 text-left hover:border-amber-200"><span className="block text-sm text-slate-700">{a.text}</span><span className="mt-1 block text-[11px] font-semibold text-emerald-700">Abrir →</span></button>)}
            {!alertas.length && <p className="rounded-xl bg-emerald-50 p-3 text-sm text-emerald-700">No hay alertas que requieran intervención.</p>}
          </section>
        </article>
      </section>
