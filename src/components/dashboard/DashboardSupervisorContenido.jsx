...
export default function DashboardSupervisorContenido() {
  ...
  const pendientes = data.tareas.filter((t) => t.estado !== "Completada"),
    atrasadas = pendientes.filter(
      (t) => t.fecha && t.fecha < new Date().toISOString().slice(0, 10),
    );
  const alertas = [
    ...data.calidad
      .filter(
        (r) =>
          (r.estado ?? r.estadoManual) !== "Cerrada" && r.prioridad === "Alta",
      )
      .map((r) => ({
        text: `Calidad: ${r.codigo || r.id} · ${r.descripcion}`,
        ruta: "/calidad",
      })),
    ...data.inventario
      .filter((r) => Number(r.stock) <= Number(r.minimo))
      .map((r) => ({
        text: `Inventario: ${r.nombre} bajo mínimo`,
        ruta: "/inventario",
      })),
    ...data.ambiental
      .filter(
        (r) =>
          Number(r.temperatura) < 18 ||
          Number(r.temperatura) > 27 ||
          Number(r.humedad) < 55 ||
          Number(r.humedad) > 80,
      )
      .map((r) => ({
        text: `Ambiental: ${r.zona} fuera de rango`,
        ruta: "/ambiental",
      })),
  ];
  const carga = [
    ...new Set(data.tareas.map((t) => t.responsable).filter(Boolean)),
  ].map((nombre) => ({
    nombre,
    total: data.tareas.filter(
      (t) => t.responsable === nombre && t.estado !== "Completada",
    ).length,
  }));
  return (
    <article className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <section>
          ...
          <p className="mt-1 max-w-2xl text-sm text-slate-500">
            Coordina la ejecución diaria: asigna trabajo, detecta bloqueos,
            valida incidencias y mantiene la operación en movimiento.
          </p>
        </section>
        <button
          type="button"
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Kpi
          ...
          detail={`${data.lotes.filter((l) => l.etapa === "Cosecha").length} en cosecha`}
        />
        <Kpi
          icon={ClipboardList}
        ...
        <Kpi
          ...
          detail="Operación, ambiente y calidad"
        ...
        <Kpi
          ...
          value={
            data.inventario.filter((r) => Number(r.stock) <= Number(r.minimo))
              .length
          }
      ...
      <section className="grid gap-4 lg:grid-cols-[1.25fr_.75fr]">
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <button
            type="button"
            onClick={() => navigate("/personal")}
            className="mt-5 inline-flex items-center gap-1 text-xs font-semibold text-emerald-700"
          >
            Gestionar tareas <ArrowRight size={12} />
          </button>
        </article>
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <header>
            ...
            <p className="mt-1 text-xs text-slate-400">
              Eventos que necesitan intervención del supervisor
            </p>
          </header>
          <section className="mt-4 space-y-2">
            {alertas.slice(0, 5).map((a) => (
              ...
              >
                <span className="block text-sm text-slate-700">{a.text}</span>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <ActionCard
          icon={Sprout}
          title="Producción"
          text="Avanza lotes, revisa etapas y crea tareas de seguimiento."
        ...
        <ActionCard
          icon={Package}
          title="Inventario"
          text="Revisa consumos, niveles mínimos y reposiciones pendientes."
        ...
        <ActionCard
          icon={Clock3}
          title="Seguimiento"
          text="Consulta trazabilidad y verifica que cada actividad quede registrada."
          action="Abrir trazabilidad"
