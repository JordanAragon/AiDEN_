...
export default function DashboardAdminContenido() {
  ...
  const data = useMemo(() => {
    ...
    const alertas = [
      ...inventario
        ...
        .map((r) => ({
          tipo: "Inventario",
          texto: `${r.nombre} está bajo el mínimo`,
          ruta: "/inventario",
        })),
      ...calidad
        .filter(
        ...
        .map((r) => ({
          tipo: "Calidad",
          texto: `${r.codigo || r.id} requiere atención`,
          ruta: "/calidad",
        })),
    ];
  ...
  const lotesAtencion = data.lotes.filter(
    (l) => l.etapa === "Adaptación",
  ).length;
  const abiertas = data.calidad.filter(
    (r) => (r.estado ?? r.estadoManual) !== "Cerrada",
  ).length;
  return (
    <article className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <section>
          <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-emerald-700">
            AiDEN / control del sistema
          </p>
          <h1 className="mt-1 text-2xl font-bold tracking-tight text-slate-950">
            Centro de administración
          </h1>
          <p className="mt-1 max-w-2xl text-sm text-slate-500">
            Supervisa usuarios, operación, seguridad y desempeño general. Aquí
            se toman decisiones del sistema, no se ejecutan tareas de campo.
          </p>
        </section>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Kpi
          ...
          detail={`${roles.supervisor} supervisores · ${roles.operario} operarios`}
        />
        <Kpi
          icon={Sprout}
          label="Lotes activos"
          ...
          detail={
            lotesAtencion
              ? `${lotesAtencion} en adaptación`
              : "Sin bloqueos de etapa"
          }
          tone={lotesAtencion ? "amber" : "green"}
        />
        <Kpi
          icon={CircleDollarSign}
          label="Balance registrado"
          value={money(data.ingresos - data.gastos)}
          detail={`${money(data.ingresos)} ingresos · ${money(data.gastos)} gastos`}
          tone="blue"
        />
        <Kpi
          icon={ShieldCheck}
          ...
          detail={
            data.alertas.length ? "Requieren seguimiento" : "Todo bajo control"
          }
          tone={data.alertas.length ? "red" : "green"}
        />
      </section>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <header>
            ...
            <p className="mt-1 text-xs text-slate-400">
              Indicadores de la información almacenada
            </p>
          </header>
          ...
          <section className="mt-5 rounded-xl bg-slate-50 p-4">
            ...
            <p className="mt-1 text-sm leading-6 text-slate-700">
              Administrar usuarios, reglas, seguridad y reportes para evaluar el
              funcionamiento global de AiDEN.
            </p>
          </section>
        </article>
          ...
          <section className="mt-4 space-y-2">
            {data.alertas.slice(0, 4).map((a) => (
              ...
              >
                ...
                <p className="mt-1 text-sm text-white/80">{a.texto}</p>
              </button>
            ))}
            {!data.alertas.length && (
      ...
      <section className="grid gap-4 lg:grid-cols-3">
              ...
              <section key={label}>
                ...
                <section className="h-2 rounded-full bg-slate-100">
                  <span
                    ...
                    style={{
                      width: `${Math.min(100, (count / Math.max(1, data.usuarios.length)) * 100)}%`,
                    }}
                  />
                </section>
              </section>
        ...
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <section className="mt-4 grid gap-2">
            <Quick onClick={() => navigate("/reportes")}>Abrir reportes</Quick>
            <Quick onClick={() => navigate("/personal")}>
              Administrar personal
            </Quick>
            <Quick onClick={() => navigate("/costos")}>Revisar costos</Quick>
            <Quick onClick={() => navigate("/configuracion")}>
              Revisar reglas
            </Quick>
          </section>
        </article>
...
function Stat({ label, value, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="rounded-xl border border-slate-200 bg-white p-4 text-left hover:border-emerald-200 hover:bg-emerald-50/30"
    >
...
function Quick({ children, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex items-center justify-between rounded-xl border border-slate-200 px-3 py-2.5 text-left text-xs font-semibold text-slate-600 hover:border-emerald-200 hover:text-emerald-700"
    >
      {children}
      <span>→</span>
    </button>
