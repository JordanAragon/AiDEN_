...
const reportes = [
  {
    ...
    descripcion: "Estado, etapa, responsable y cantidad de plantas.",
    icon: Sprout,
  },
  {
    id: "inventario",
    titulo: "Inventario y reposición",
    descripcion: "Existencias, mínimos y artículos que requieren atención.",
    icon: Package,
  },
  {
    id: "calidad",
    titulo: "Incidencias de calidad",
    descripcion: "Casos abiertos, prioridad y responsables.",
    icon: ShieldCheck,
  },
  {
    id: "ambiental",
    titulo: "Resumen ambiental",
    descripcion: "Última lectura disponible por zona y alertas.",
    icon: Leaf,
  },
  {
    id: "personal",
    titulo: "Carga de trabajo",
    descripcion: "Tareas pendientes y completadas por colaborador.",
    icon: Users,
  },
  {
    id: "costos",
    titulo: "Costos por lote",
    descripcion: "Gasto acumulado, costo por planta y balance registrado.",
    icon: BarChart3,
  },
];
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        <section>
          ...
          <p className="mt-1 max-w-2xl text-sm text-slate-500">
            Consulta información consolidada de los módulos. Cada reporte se
            construye con los datos actuales de AiDEN.
          </p>
        </section>
        {rows.length > 0 && (
          <button
            type="button"
            onClick={() => exportar(selected, rows)}
            className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-semibold text-slate-600 hover:border-emerald-200 hover:text-emerald-700"
          >
            <Download size={15} />
            Exportar CSV
          </button>
        ...
        {reportes.map((r) => {
          ...
          return (
            <button
              ...
              className={`rounded-2xl border p-4 text-left transition ${selected === r.id ? "border-emerald-300 bg-emerald-50/50" : "border-slate-200 bg-white hover:border-emerald-200"}`}
            >
        ...
        <header className="flex items-center gap-3 border-b border-slate-100 p-5">
          ...
          <section>
            ...
            <p className="text-xs text-slate-400">
              {rows.length} registros disponibles
            </p>
          </section>
        </header>
          ...
          <table className="w-full min-w-[760px]">
            ...
            <tbody>
              ...
              {!rows.length && (
                <tr>
                  <td
                    colSpan={Object.keys(rows[0] || { Reporte: 1 }).length}
                    className="px-4 py-12 text-center text-sm text-slate-400"
                  >
                    No hay datos registrados para generar este reporte.
                  </td>
                </tr>
              )}
            </tbody>
