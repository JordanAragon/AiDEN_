...
import { BarChart3, BrainCircuit, ChevronLeft, ChevronRight, CircleDollarSign, GitBranch, LayoutDashboard, Leaf, Moon, Package, Settings, ShieldCheck, Sprout, Sun, Users, Thermometer } from "lucide-react";
...
export default function BarraLateral(){
 const [colapsado,setColapsado]=useState(()=>localStorage.getItem("aiden-sidebar")==="collapsed");
 const [oscuro,setOscuro]=useState(()=>localStorage.getItem("aiden-theme")==="dark");
 const location=useLocation(); const session=getSession(); const role=session?.role||"operario"; const pathDashboard=getDashboardPath(role);
 const visible=useMemo(()=>navItems.filter(item=>item.roles.includes(role)),[role]);
