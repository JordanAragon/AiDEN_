...
export default function BarraSuperior() {
  ...
  const refrescar = () => {
    ...
    const source =
      role === "operario"
        ? [
            ...propios
              .filter((r) => r.etapa === "Cosecha")
              .map((r) => ({
                id: `lot-${r.id}`,
                text: `${r.lote} está listo para cosecha`,
                ruta: "/produccion",
              })),
            ...calidad
              .filter(
                (r) =>
                  r.estado !== "Cerrada" &&
                  r.prioridad === "Alta" &&
                  propios.some((l) => l.lote === r.lote),
              )
              .map((r) => ({
                id: `cal-${r.id}`,
                text: `Incidencia alta en ${r.lote || r.codigo}`,
                ruta: "/calidad",
              })),
            ...ambiental
              .filter(
              ...
              .map((r) => ({
                id: `amb-${r.id}`,
                text: `Condición ambiental fuera de rango: ${r.zona}`,
                ruta: "/ambiental",
              })),
          ]
        : [
            ...
            ...produccion
              ...
              .map((r) => ({
                id: `lot-${r.id}`,
                text: `${r.lote} listo para cosecha`,
                ruta: "/produccion",
              })),
            ...ambiental
              .filter(
  ...
  const marcarLeida = (item) => {
    setNotificaciones((xs) =>
      xs.map((n) => (n.id === item.id ? { ...n, read: true } : n)),
    );
    try {
      const actual = JSON.parse(
        localStorage.getItem(CLAVE_NOTIFICACIONES) || "[]",
      );
      if (!actual.includes(item.id))
        localStorage.setItem(
          CLAVE_NOTIFICACIONES,
          JSON.stringify([...actual, item.id]),
        );
    } catch {}
    navigate(item.ruta);
  };
  const marcarTodas = () => {
    ...
    try {
      localStorage.setItem(
        CLAVE_NOTIFICACIONES,
        JSON.stringify(notificaciones.map((n) => n.id)),
      );
    } catch {}
  };
  const handleLogout = () => {
            ...
            {resultados.length ? (
              resultados.slice(0, 7).map((item) => (
                <button
                  ...
                  className="flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-left text-sm text-slate-700 hover:bg-emerald-50 hover:text-emerald-700"
                >
      ...
      <section className="flex items-center gap-3">
        <section className="relative">
          ...
          >
            ...
            {noLeidas > 0 && (
              <span className="absolute right-0.5 top-0.5 min-w-4 rounded-full bg-red-500 px-1 text-[9px] font-bold leading-4 text-white">
                {noLeidas}
              </span>
            )}
          </button>
          {showNotifs && (
            <section
              className="absolute right-0 top-12 z-30 w-80 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl"
              role="dialog"
            >
                  ...
                  notificaciones.map((n) => (
                    <li key={n.id}>
                      <button
                        ...
                        className={`w-full px-4 py-3 text-left hover:bg-slate-50 ${n.read ? "opacity-50" : ""}`}
                      >
                        <span className="flex gap-3">
                          <span
                            className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${n.read ? "bg-slate-300" : "bg-emerald-500"}`}
                          />
        ...
        <section className="relative">
          <button
            ...
            className="flex items-center gap-2 rounded-xl py-1.5 pl-2 pr-3 hover:bg-slate-100"
          >
            ...
            <section className="hidden text-left sm:block">
              ...
              <p className="mt-0.5 text-xs text-slate-400">{rol}</p>
            </section>
            <ChevronDown size={14} className="text-slate-400" />
          </button>
          {showProfile && (
            <section
              className="absolute right-0 top-12 z-30 w-56 rounded-2xl border border-slate-200 bg-white py-2 shadow-xl"
              role="dialog"
            >
