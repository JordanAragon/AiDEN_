...
function Input({ label, ...props }) {
  return (
    <label className="block text-sm font-medium text-slate-600">
      ...
      <input
        {...props}
        className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm text-slate-800 outline-none focus:border-emerald-500"
      />
    </label>
  );
}
...
function Textarea({ label, ...props }) {
  return (
    <label className="block text-sm font-medium text-slate-600">
      ...
      <textarea
        {...props}
        className="mt-1 min-h-24 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm text-slate-800 outline-none focus:border-emerald-500"
      />
    </label>
  );
}
...
export default function CalidadOperativo() {
  ...
  const canResolve = role !== "operario";
  ...
  const open = rows.filter((r) => (r.estado ?? r.estadoManual) !== "Cerrada");
  const high = open.filter((r) => r.prioridad === "Alta");
  const filtered = useMemo(
    () =>
      rows.filter(
        (r) =>
          (filter === "Todas" || (r.estado ?? r.estadoManual) === filter) &&
          `${r.codigo} ${r.lote} ${r.descripcion} ${r.responsable}`
            .toLowerCase()
    ...
    <section className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <section>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Registra una desviación, deja claro quién la atiende y conserva su
            resolución.
          </p>
        </section>
        <button
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Kpi
          label="Resueltas"
          value={
            rows.filter((r) => (r.estado ?? r.estadoManual) === "Cerrada")
              .length
          }
          detail="Cerradas"
          icon={CheckCircle2}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <header className="flex flex-wrap items-center gap-3 border-b border-slate-100 p-4">
          ...
          {["Todas", "Abierta", "En revisión", "Cerrada"].map((s) => (
            <button
              key={s}
              type="button"
              onClick={() => setFilter(s)}
              className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${filter === s ? "bg-emerald-700 text-white" : "bg-slate-100 text-slate-500"}`}
            >
              {s}
            </button>
          ))}
        ...
        <section className="grid gap-3 p-4">
          {filtered.map((r) => {
            const estado = r.estado ?? r.estadoManual;
            return (
              <article
                key={r.id}
                ...
                <header className="flex flex-wrap items-start justify-between gap-3">
                  <section>
                    ...
                    <p className="mt-1 text-xs text-slate-400">
                      {r.responsable} · {r.fecha}
                    </p>
                  </section>
                  <section className="flex gap-2">
                    <span
                      className={`rounded-full px-2 py-1 text-[10px] font-bold ${r.prioridad === "Alta" ? "bg-red-50 text-red-700" : r.prioridad === "Media" ? "bg-amber-50 text-amber-700" : "bg-slate-100 text-slate-500"}`}
                    >
                      {r.prioridad}
                    </span>
                    {canResolve ? (
                      <select
                        value={estado}
                        onChange={(e) => update(r, "estado", e.target.value)}
                ...
                <section className="mt-4 grid gap-3 md:grid-cols-[1fr_auto]">
                  {canResolve ? (
                    <Input
                      ...
                      value={r.accion || ""}
                      onChange={(e) => update(r, "accion", e.target.value)}
                    ...
                    <section className="rounded-xl bg-slate-50 p-3 text-sm">
                      ...
                      <p className="mt-1 text-sm text-slate-700">
                        {r.accion || "Pendiente de definir por supervisión."}
                      </p>
                    </section>
                  )}
                  <button
                    ...
                    className="self-end rounded-xl bg-slate-950 px-3 py-2.5 text-xs font-semibold text-white"
                  >
                    Ver detalle
                  </button>
                </section>
      ...
      {selected && (
        <Modal title={selected.codigo} onClose={() => setSelected(null)}>
          <section className="space-y-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Lote {selected.lote} · responsable {selected.responsable}
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <Kpi
                ...
                icon={AlertTriangle}
                tone={selected.prioridad === "Alta" ? "red" : "amber"}
              />
              <Kpi
                label="Estado"
                value={selected.estado ?? selected.estadoManual}
                icon={ClipboardCheck}
              />
            </div>
            <section className="rounded-xl bg-slate-50 p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-700">
                {selected.accion || "Todavía no documentada."}
              </p>
            </section>
          </section>
        </Modal>
      ...
      {modal && (
        <Modal title="Nueva incidencia" onClose={() => setModal(false)}>
          <QualityForm people={people} lots={lots} onSubmit={create} />
        </Modal>
      )}
    </section>
  );
}
function QualityForm({ people, lots, onSubmit }) {
  const [f, setF] = useState({
    lote: lots[0]?.lote || "",
    prioridad: "Media",
    descripcion: "",
    responsable: people[0]?.nombre || "",
  });
  return (
    <form
      className="space-y-4"
