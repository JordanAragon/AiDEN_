...
const seed = [
  {
    id: "CST-001",
    lote: "LT-2024-089",
    concepto: "Sustrato y fertilización",
    categoria: "Insumos",
    valor: 318000,
    fecha: "2026-09-03",
    tipo: "gasto",
  ...
  },
  {
    id: "CST-003",
    lote: "LT-2024-094",
    concepto: "Jornada de adecuación",
...
export default function CostosOperativo() {
  ...
  const porLote = useMemo(
    () =>
      lotes
        .map((l) => {
          const gasto = rows
            .filter((r) => r.lote === l.lote && r.tipo === "gasto")
            .reduce((a, r) => a + Number(r.valor || 0), 0);
          const ingreso = rows
            .filter((r) => r.lote === l.lote && r.tipo === "ingreso")
            .reduce((a, r) => a + Number(r.valor || 0), 0);
          return {
            lote: l.lote,
            gasto,
            ingreso,
            plantas: Number(l.cantidad || 0),
            costoPlanta: l.cantidad ? gasto / Number(l.cantidad) : 0,
            resultado: ingreso - gasto,
          };
        })
        .filter((r) => r.gasto || r.ingreso),
    [lotes, rows],
  );
  const categorias = Object.entries(
    gastos.reduce((a, r) => {
      a[r.categoria] = (a[r.categoria] || 0) + Number(r.valor || 0);
      return a;
    }, {}),
  ...
  const add = (f) => {
    save([{ ...f, id: uid(), valor: Number(f.valor) }, ...rows]);
    setModal(false);
  };
  const download = () => {
    const blob = new Blob(
      [JSON.stringify({ gastos, ingresos, balance, porLote }, null, 2)],
      { type: "application/json" },
    );
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "aiden-costos.json";
    a.click();
    ...
    <section className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <section>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Registra movimientos y convierte cada gasto en información por lote
            y por planta.
          </p>
        </section>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Kpi
          ...
          detail={`${gastos.length} movimientos`}
          icon={TrendingDown}
          tone="red"
        />
        <Kpi
          ...
          detail={`${ingresos.length} movimientos`}
          icon={TrendingUp}
        />
        <Kpi
          label="Balance"
          value={money(balance)}
          detail={
            balance >= 0 ? "Ingresos - gastos" : "Gastos superiores a ingresos"
          }
          icon={CircleDollarSign}
          tone={balance >= 0 ? "green" : "red"}
        />
        <Kpi
          label="Costo/planta medio"
          value={
            porLote.length
              ? money(
                  porLote.reduce((a, r) => a + r.costoPlanta, 0) /
                    porLote.length,
                )
              : "$0"
          }
          detail={`${porLote.length} lotes con costos`}
          icon={BarChart3}
          tone="blue"
        />
      </section>
      <section className="grid gap-4 lg:grid-cols-[1.35fr_.65fr]">
        <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <header>
            ...
            <p className="text-xs text-slate-400">
              Solo se incluyen gastos registrados
            </p>
          </header>
          <section className="mt-4 h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={categorias}>
                ...
                />
                <Tooltip formatter={(v) => [money(v), "Gasto"]} />
                <Bar dataKey="total" fill="#176b45" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </section>
        </article>
        <article className="rounded-2xl border border-slate-200 bg-slate-950 p-5 text-white">
          ...
          <section className="mt-4 space-y-3">
            {porLote.slice(0, 5).map((r) => (
              <section key={r.lote} className="rounded-xl bg-white/5 p-3">
                <div className="flex justify-between gap-3">
                  ...
                  <span
                    className={
                      r.resultado >= 0 ? "text-emerald-300" : "text-red-300"
                    }
                  >
                    {money(r.resultado)}
                  </span>
                ...
                <p className="mt-1 text-[11px] text-white/45">
                  Costo/planta: {money(r.costoPlanta)}
                </p>
              </section>
            ))}
            {porLote.length === 0 && (
              <p className="text-sm text-white/45">
                Asocia movimientos a lotes para calcular rentabilidad.
              </p>
            )}
          </section>
        </article>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 p-4">
          <section>
            ...
            <p className="text-xs text-slate-400">
              Fuente de todos los cálculos anteriores
            </p>
          </section>
          <section className="flex gap-1">
            {["Todos", "gasto", "ingreso"].map((v) => (
              <button
                key={v}
                type="button"
                onClick={() => setFiltro(v)}
                className={`rounded-lg px-3 py-1.5 text-xs font-semibold ${filtro === v ? "bg-emerald-700 text-white" : "bg-slate-100 text-slate-500"}`}
              >
                {v}
              </button>
            ))}
              ...
              {filtered.map((r) => (
                <tr key={r.id} className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`rounded-full px-2 py-1 text-[10px] font-bold ${r.tipo === "ingreso" ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"}`}
                    >
                      {r.tipo}
                    </span>
                  </td>
                  <td
                    className={`px-4 py-3 text-sm font-bold ${r.tipo === "ingreso" ? "text-emerald-700" : "text-red-600"}`}
                  >
                    {r.tipo === "ingreso" ? "+" : "-"}
                    {money(r.valor)}
                  </td>
      ...
      {modal && (
        <Modal onClose={() => setModal(false)}>
          <CostForm lotes={lotes} onSubmit={add} />
        </Modal>
      )}
    </section>
  );
