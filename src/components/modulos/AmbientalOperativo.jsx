...
function rules() {
  return { ...defaults, ...read(CONFIG, defaults) };
}
    ...
    <section className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 p-4 backdrop-blur-sm">
      <article className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <header className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
          <h2 className="font-semibold text-slate-900">Nueva lectura</h2>
          <button
            type="button"
            onClick={onClose}
...
export default function AmbientalOperativo() {
  ...
  const cfg = useMemo(() => rules(), [configVersion]);
  useEffect(() => {
    const handler = () => setConfigVersion((v) => v + 1);
    window.addEventListener("aiden-config-change", handler);
    return () => window.removeEventListener("aiden-config-change", handler);
  }, []);
  const save = (data) => {
    setRows(data);
    write(KEY, data);
  ...
  const zones = [...new Set(rows.map((r) => r.zona))];
  const latest = zones.map(
    (z) =>
      rows
        .filter((r) => r.zona === z)
  ...
  const outside = (r) =>
    Number(r.temperatura) < cfg.tempMin ||
    Number(r.temperatura) > cfg.tempMax ||
    Number(r.humedad) < cfg.humMin ||
    Number(r.humedad) > cfg.humMax;
  const alerts = latest.filter(outside);
  const filtered =
    zone === "Todas" ? latest : latest.filter((r) => r.zona === zone);
  const add = (f) => {
    save([
      {
        ...f,
        ...
        humedad: Number(f.humedad),
        iluminacion: Number(f.iluminacion),
      },
      ...rows,
    ]);
    setModal(false);
    ...
    <section className="space-y-6">
      <header className="flex flex-wrap items-start justify-between gap-4">
        <section>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Monitorea condiciones por zona, detecta desviaciones y conserva
            histórico para actuar a tiempo.
          </p>
        </section>
        <button
          type="button"
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Kpi
          label="Zonas"
          value={zones.length}
          detail="Con lecturas registradas"
          icon={Thermometer}
        />
        <Kpi
          label="En alerta"
          value={alerts.length}
          detail={`Rango ${cfg.tempMin}–${cfg.tempMax} °C · ${cfg.humMin}–${cfg.humMax}%`}
          icon={AlertTriangle}
        ...
        <Kpi
          ...
          value={
            rows[0]
              ? new Date(rows[0].fecha).toLocaleTimeString("es-CO", {
                  hour: "2-digit",
                  minute: "2-digit",
                })
              : "—"
          }
          detail="Última lectura"
          icon={Clock3}
          tone="amber"
        />
      </section>
      <section className="flex gap-1 overflow-x-auto">
        {["Todas", ...zones].map((z) => (
          <button
            ...
            type="button"
            onClick={() => setZone(z)}
            className={`whitespace-nowrap rounded-lg px-3 py-1.5 text-xs font-semibold ${zone === z ? "bg-emerald-700 text-white" : "bg-slate-100 text-slate-500"}`}
          >
            {z}
          </button>
        ))}
      ...
      {alerts.length > 0 && (
        <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
          ...
          <p className="mt-1 text-xs text-amber-800">
            La alerta se calcula con los umbrales definidos en Configuración.
          </p>
        </section>
      )}
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {filtered.map((r) => {
          const alert = outside(r);
          const history = rows
            .filter((x) => x.zona === r.zona)
            .slice(0, 10)
            .reverse();
          return (
            <article
            ...
            >
              ...
              <section className="mt-5 grid grid-cols-3 gap-2">
                ...
                <Metric
                  label="Luz"
                  value={`${Number(r.iluminacion).toLocaleString("es-CO")} lux`}
                />
              </section>
              ...
              <footer className="mt-3 border-t border-slate-100 pt-3 text-[11px] text-slate-400">
                Última lectura: {new Date(r.fecha).toLocaleString("es-CO")}
              </footer>
            </article>
          );
        })}
      ...
      {modal && (
        <Modal onClose={() => setModal(false)}>
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              add(Object.fromEntries(new FormData(e.currentTarget).entries()));
            }}
          >
            <Input
              name="zona"
            ...
            <section className="grid grid-cols-3 gap-2">
              <Input
                ...
                required
              />
              <Input name="humedad" label="Humedad %" type="number" required />
              <Input
                name="iluminacion"
...
function Metric({ label, value, danger = false }) {
  return (
    <section
      className={`rounded-xl p-3 ${danger ? "bg-red-50" : "bg-slate-50"}`}
    >
      <p className="text-[10px] text-slate-400">{label}</p>
      <p
        className={`mt-1 text-sm font-bold ${danger ? "text-red-700" : "text-slate-800"}`}
      >
        {value}
      </p>
    </section>
